...
from src.utils.comm import is_main_process, get_rank, get_world_size, dist_init
...
def mixed_precision_init(args, model):
    ...
    optimizer_grouped_parameters = [
        ...
        {"params": [p for n, p in decay_bert_param_tp], "weight_decay": weight_decay},
        {
            "params": [p for n, p in no_decay_swin_param_tp],
            "weight_decay": 0.0,
            "lr": args.learning_rate * coef_lr,
        },
        {"params": [p for n, p in no_decay_bert_param_tp], "weight_decay": 0.0},
    ]
    ...
    if args.mixed_precision_method == "deepspeed":
        ...
        model = ShardedDDP(
            ...
            if args.fairscale_fp16
            else 2**23,  # 2 ** 23 is the default value
            reduce_fp16=args.fairscale_fp16,
        )
    else:
        # opt_level is O0, Apex will run as fp32
        model, optimizer = amp.initialize(
            model, optimizer, enabled=True, opt_level=f"O{args.amp_opt_level}"
        )
        if args.distributed:  #
            model = DDP(model)
...
def train(
    ...
    max_iter = args.max_iter = 2000

    max_global_step = args.max_global_step = 2000
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        if args.mixed_precision_method == "deepspeed":
            ...
            # apex
            with amp.scale_loss(
                loss, optimizer, delay_unscale=not backward_now
            ) as scaled_loss:
        ...
        # memory allocating for evaluation
        maybe_batch_size = batch[0]
        del img_keys, meta_data, inputs, outputs, batch
        gc.collect()
        ...
        if backward_now:
            ...
            if (
                ...
                or global_step == max_global_step
                or global_step == 1
            ):
                epoch = global_step // global_iters_per_epoch

                checkpoint_dir = op.join(
                    args.output_dir, "checkpoint-{}-{}".format(epoch, global_step)
                )
                if get_world_size() > 1:
                    dist.barrier()
                ...
                if args.evaluate_during_training:
                    ...
                    evaluate_file = evaluate(
                        args, val_dataloader, model, tokenizer, checkpoint_dir
                    )
                    ...
                    if is_main_process():
                        ...
                        with open(
                            op.join(
                                args.output_dir,
                                args.val_yaml.replace("/", "_") + "eval_logs.json",
                            ),
                            "w",
                        ) as f:
...
def test(args, test_dataloader, model, tokenizer, predict_file):
    ...
    if world_size == 1:
        ...
        cache_file = (
            ...
            + "_{}_{}".format(get_rank(), world_size)
    ...
    def gen_rows():
        ...
        with torch.no_grad():
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                del batch, meta_data, inputs, outputs
                gc.collect()
                ...
                del img_keys, all_caps, all_confs
                gc.collect()
...
def check_arguments(args):
    ...
    args.max_img_seq_length = int(
        (args.max_num_frames / 2) * (int(args.img_res) / 32) * (int(args.img_res) / 32)
    )
...
def get_custom_args(base_config):
    ...
    parser.add_argument(
        "--grid_feat", type=str_to_bool, nargs="?", const=True, default=True
    )
    parser.add_argument("--kinetics", type=str, default="400", help="400 or 600")
    parser.add_argument(
        "--pretrained_2d", type=str_to_bool, nargs="?", const=True, default=False
    )
    parser.add_argument("--vidswin_size", type=str, default="base")
    parser.add_argument(
        "--freeze_backbone", type=str_to_bool, nargs="?", const=True, default=False
    )
    parser.add_argument(
        "--use_checkpoint", type=str_to_bool, nargs="?", const=True, default=False
    )
    parser.add_argument("--backbone_coef_lr", type=float, default=0.001)
    parser.add_argument(
        "--reload_pretrained_swin",
        type=str_to_bool,
        nargs="?",
        const=True,
        default=False,
    ...
    parser.add_argument(
        "--learn_mask_enabled", type=str_to_bool, nargs="?", const=True, default=False
    )
...
def main(args):
            ...
            # transfer across different frame rates
                ...
                if args.transfer_method == 0:
                    if isinstance(pretrained_model, dict):
                        new_state_dict = {}
                        for k, v in zip(
                            pretrained_model.keys(), pretrained_model.values()
                        ):
                            if (
                                k != "learn_vid_att.weight"
                                or k == "learn_vid_att.weight"
                                and pretrained_mask_shape == init_mask_shape
                            ):
                                new_state_dict = {k: v}
                        ...
                        new_state_dict = {}
                        for k, v in zip(
                            pretrained_model_state_dict.keys(),
    ...
    if args.do_train:
        ...
        train_dataloader = make_data_loader(
            args, args.train_yaml, tokenizer, args.distributed, is_train=True
        )
        val_dataloader = make_data_loader(
            args, args.val_yaml, tokenizer, args.distributed, is_train=False
        )
        ...
        evaluate_file = evaluate(
            args, val_dataloader, vl_transformer, tokenizer, args.eval_model_dir
        )
...
if __name__ == "__main__":
    ...
    wandb.init(project="SwinBERT-finetuning", name=args.model_name_or_path)
