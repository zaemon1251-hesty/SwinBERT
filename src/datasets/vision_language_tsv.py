...
from .data_utils.video_ops import (
    extract_frames_from_video_binary,
...
class VisionLanguageTSVDataset(object):
    def __init__(
        ...
        self.decoder_sampling_strategy = getattr(
            args, "decoder_sampling_strategy", "uniform"
        )
        LOGGER.info(
            f"isTrainData: {self.is_train}\n[PyAV video parameters] "
...
>>>>>>> Stashed changes
    ...
    def apply_augmentations(self, frames):
        ...
        if frames is None:
            frames = np.zeros(
                (self.decoder_num_frames, self.img_res, self.img_res, 3)
            ).astype(np.uint8)
    ...
    def __getitem__(self, idx):
        ...
        if self.args.add_od_labels == True:
            example = self.tensorizer.tensorize_example_e2e(
                caption, preproc_frames, text_b=tag, text_meta=caption_sample
            )
        else:
            example = self.tensorizer.tensorize_example_e2e(
                caption, preproc_frames, text_meta=caption_sample
            )
